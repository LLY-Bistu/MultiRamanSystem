...
class LearnableWaveletTransform(nn.Module):
    def __init__(self, wavelet_size=3):
        super(LearnableWaveletTransform, self).__init__()
        # 初始化可学习的小波滤波器
        self.filters = nn.Parameter(torch.randn(1, 1, wavelet_size))  # (out_channels, in_channels, kernel_size)

    def forward(self, x):
        # 进行卷积操作
        return F.conv1d(x, self.filters, stride=1, padding=1)

@BACKBONES.register_module()
class MTMamba(BaseBackbone):
    def __init__(self, d_input=900, d_model=900, n_layer=1, d_state=16, d_conv=4, expand=2, dt_rank='auto',
                 conv_bias=True, bias=False):
        super().__init__()

        self.conv1 = nn.Sequential(
            nn.Conv1d(in_channels=d_input * 2, out_channels=d_input, kernel_size=7, stride=2, padding=3, bias=False),
        ...
        self.layers = nn.ModuleList([ResidualBlock(self.d_model, self.d_inner, self.d_state, self.d_conv, self.dt_rank,
                                                   self.conv_bias, self.bias) for _ in range(self.n_layer)])
